...
def test_implementing_devices(module, capsys, expected_scan_output):
    ...
    try:
        with patch("matplotlib.get_backend"):
            with patch(
                "bluesky.run_engine.autoawait_in_bluesky_event_loop"
            ) as autoawait:
                main = importlib.import_module(f"{module}.__main__")
                autoawait.assert_called_once_with()

            # We want the text output of the best effort callback, but the plotting
            # take too much time for CI, even if headless, so disable it
            ...
            RE(
                main.bp.grid_scan(
                    [main.pdet], main.stage.x, 1, 2, 3, main.stage.y, 2, 3, 3
                )
            )
        ...
        # Ensure all tasks are awaited and the loop is stopped cleanly
